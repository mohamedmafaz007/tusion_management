...
//#region node_modules/.nitro/vite/services/ssr/assets/db-BfFeuBvY.js
...
var getDbStudents = createServerFn({ method: "GET" }).handler(getDbStudents_createServerFn_handler, async () => {
	...
	try {
		return (await sql`SELECT * FROM students`).map((r) => ({
			id: r.id,
			registrationNo: r.registrationNo || `VTC-${r.id.slice(0, 6).toUpperCase()}`,
			photo: r.photo,
			name: r.name,
			gender: r.gender,
			dob: r.dob,
			...
			boardOfStudy: r.boardOfStudy,
			mediumOfStudy: r.mediumOfStudy,
			notes: r.notes,
...
async function getWhatsAppService() {
	if (typeof window === "undefined") return await import("./whatsappService-6q71eFu2.mjs");
	return null;
}
...
var sendWhatsAppReceipt = createServerFn({ method: "POST" }).validator((data) => data).handler(sendWhatsAppReceipt_createServerFn_handler, async ({ data }) => {
	...
	let sentCount = 0;
	let lastError = null;
	for (const phone of parentPhones) try {
		await ws.sendWhatsAppTextMessage(phone, textMessage);
		await ws.sendWhatsAppMessageWithPDF(phone, "Fee Payment Receipt PDF", pdfBuffer, `Receipt_${f.id.slice(0, 8).toUpperCase()}.pdf`);
		await logMessage({
			type: "fee_receipt",
			studentId: s.id,
			studentName: s.name,
	...
	if (sentCount === 0 && lastError) throw lastError;
	return { success: true };
});
...
var sendBulkAttendanceAlerts = createServerFn({ method: "POST" }).validator((data) => data).handler(sendBulkAttendanceAlerts_createServerFn_handler, async ({ data }) => {
	...
	for (const rec of attendanceRecords) {
		...
		for (const phone of parentPhones) {
			try {
				if (provider === "baileys") await ws.sendWhatsAppTextMessage(phone, messageText);
				await logMessage({
					type: "attendance",
					studentId: rec.sid,
...
var FESTIVALS = {
	"2026-01-01": {
		name: "New Year's Day",
		wish: "Happy New Year! May this year bring joy, peace, and success to you and your family. 🎉"
	},
	"2026-01-14": {
		name: "Pongal / Makar Sankranti",
		wish: "Wishing you a very Happy Pongal and Makar Sankranti! May the harvest season bring abundance and sweet moments. 🌾"
	},
	"2026-01-26": {
		name: "Republic Day",
		wish: "Happy Republic Day! Let us salute the nation and honor our constitution. 🇮🇳"
	},
	"2026-02-15": {
		name: "Maha Shivaratri",
		wish: "Wishing you a blessed Maha Shivaratri. May Lord Shiva bless you with peace and wisdom. 🔱"
	},
	"2026-03-03": {
		name: "Holi",
		wish: "Happy Holi! May your life be filled with vibrant colors of happiness, love, and laughter. 🎨"
	},
	"2026-03-20": {
		name: "Eid al-Fitr",
		wish: "Eid Mubarak! Wishing you and your family a blessed day filled with joy and happiness. 🌙"
	},
	"2026-04-03": {
		name: "Good Friday",
		wish: "Have a blessed Good Friday. May the love and grace of God shine upon you today and always. 🕯️"
	},
	"2026-04-14": {
		name: "Tamil New Year / Puthandu",
		wish: "Iniya Puthandu Vazhthukkal! May this Tamil New Year bring health, prosperity, and happiness. 🍊🎉"
	},
	"2026-05-27": {
		name: "Eid al-Adha",
		wish: "Eid al-Adha Mubarak! May your faith and devotion be rewarded with happiness, peace, and blessings. 🌙"
	},
	"2026-08-15": {
		name: "Independence Day",
		wish: "Happy Independence Day! Let us celebrate the freedom and unity of our great nation. 🇮🇳"
	},
	"2026-09-04": {
		name: "Krishna Janmashtami",
		wish: "Happy Janmashtami! May Lord Krishna steal all your worries and bless you with happiness and love. 🍯"
	},
	"2026-09-15": {
		name: "Ganesh Chaturthi",
		wish: "Happy Ganesh Chaturthi! May Lord Ganesha bless you with intelligence, prosperity, and success. 🐘"
	},
	"2026-10-02": {
		name: "Gandhi Jayanti",
		wish: "Happy Gandhi Jayanti! Let us remember and honor the father of our nation and follow the path of truth. 👓"
	},
	"2026-10-20": {
		name: "Vijayadashami / Dussehra",
		wish: "Happy Dussehra! May the victory of good over evil bring joy, peace, and prosperity to you. 🏹"
	},
	"2026-11-08": {
		name: "Diwali",
		wish: "Wishing you a very Happy and Safe Diwali! May the festival of lights illuminate your life with happiness and wealth. 🪔"
	},
	"2026-12-25": {
		name: "Christmas",
		wish: "Merry Christmas! May the holiday season fill your home with joy, love, and laughter. 🎄"
	},
	"2027-01-01": {
		name: "New Year's Day",
		wish: "Happy New Year! May this year bring joy, peace, and success to you and your family. 🎉"
	...
	"2028-04-14": {
		name: "Tamil New Year / Puthandu & Good Friday",
		wish: "Iniya Puthandu Vazhthukkal and a blessed Good Friday! May this day bring health, peace, prosperity, and blessings to you. 🍊🎉🕯️"
	},
	"2028-05-05": {
		name: "Eid al-Adha",
		wish: "Eid al-Adha Mubarak! May your faith and devotion be rewarded with happiness, peace, and blessings. 🌙"
...
var checkAndSendFestivalGreetings_createServerFn_handler = createServerRpc({
	id: "2bb2e6ec9cbe835d7c74c0ae53952b0d552c165a6b95de3f812eb789f5adf0f3",
	name: "checkAndSendFestivalGreetings",
	filename: "src/lib/db.ts"
}, (opts) => checkAndSendFestivalGreetings.__executeServer(opts));
var checkAndSendFestivalGreetings = createServerFn({ method: "POST" }).handler(checkAndSendFestivalGreetings_createServerFn_handler, async () => {
	const sql = await getSql();
	if (!sql) throw new Error("Database not connected");
	const settings = await getDbSettings();
	if (!settings) throw new Error("Settings not configured");
	...
	const ws = await getWhatsAppService();
	if (!ws) throw new Error("WhatsApp service not available");
	const festival = FESTIVALS[(/* @__PURE__ */ new Date()).toLocaleDateString("en-CA", { timeZone: "Asia/Kolkata" })];
	if (!festival) return {
		success: true,
		festival: null,
		sent: 0
...
var loginAdmin_createServerFn_handler = createServerRpc({
	id: "88830e17169b508f96bdac39016eacc7a7078dd59237ad345b1c7ab4c73f2f3c",
	name: "loginAdmin",
	filename: "src/lib/db.ts"
}, (opts) => loginAdmin.__executeServer(opts));
var loginAdmin = createServerFn({ method: "POST" }).validator((password) => password).handler(loginAdmin_createServerFn_handler, async ({ data: password }) => {
	const sql = await getSql();
...
var sendPasswordResetOtp_createServerFn_handler = createServerRpc({
	id: "471c3a8ff6048475296905e7e3859693651747eba892caa43ae896a97abeef96",
	name: "sendPasswordResetOtp",
	filename: "src/lib/db.ts"
}, (opts) => sendPasswordResetOtp.__executeServer(opts));
var sendPasswordResetOtp = createServerFn({ method: "POST" }).handler(sendPasswordResetOtp_createServerFn_handler, async () => {
	const sql = await getSql();
	if (!sql) {
		console.warn("Database not configured. Cannot verify OTP.");
		return {
			success: false,
			error: "Database not configured. Cannot use OTP verification."
		};
	}
	...
	try {
		await sql`
        INSERT INTO settings ("key", "value")
        VALUES ('reset_otp_session', ${JSON.stringify({
			otp,
			expiresAt
		})})
        ON CONFLICT ("key") DO UPDATE SET "value" = EXCLUDED."value"
      `;
		...
		return {
			success: false,
			error: "Failed to store OTP session"
		};
	}
	console.log(`\n==================================================`);
	...
	console.log(`==================================================\n`);
	try {
		const nodemailer = await import("../_libs/nodemailer.mjs").then((n) => /* @__PURE__ */ __toESM(n.t()));
...
var verifyOtpAndResetPassword_createServerFn_handler = createServerRpc({
	id: "c028d986e22e025240b6bf878835686494cbae99a4d00e5d3b0e13ffe10ad46b",
	name: "verifyOtpAndResetPassword",
	filename: "src/lib/db.ts"
}, (opts) => verifyOtpAndResetPassword.__executeServer(opts));
var verifyOtpAndResetPassword = createServerFn({ method: "POST" }).validator((data) => data).handler(verifyOtpAndResetPassword_createServerFn_handler, async ({ data: { otp, newPassword } }) => {
	const sql = await getSql();
	if (!sql) return {
		success: false,
	...
	try {
		...
		if (rows.length === 0) return {
			success: false,
			error: "No active password reset session found. Please request a new OTP."
		};
...
var getRegistrationPdf_createServerFn_handler = createServerRpc({
	id: "f7c747edd2ef4795fc6f4445ec4a40f0fc5cb3c3f9beed62f6e2224d2f040fa3",
	name: "getRegistrationPdf",
	filename: "src/lib/db.ts"
}, (opts) => getRegistrationPdf.__executeServer(opts));
var getRegistrationPdf = createServerFn({ method: "POST" }).validator((data) => data).handler(getRegistrationPdf_createServerFn_handler, async ({ data }) => {
	const sql = await getSql();
	if (!sql) throw new Error("Database not connected");
	const settings = await getDbSettings();
	if (!settings) throw new Error("Settings not configured");
	...
	if (!ws) throw new Error("WhatsApp service not available");
	return {
		pdfBase64: (await ws.generateRegistrationPdf({
			instituteName: settings.instituteName || "Vishwa Tuition Center",
			address: settings.address || "",
			contact: settings.contact || "",
			student: {
...
//#endregion
export { checkAndSendBirthdayWishes_createServerFn_handler, checkAndSendFestivalGreetings_createServerFn_handler, clearAllDbMessageLogs_createServerFn_handler, connectBaileys_createServerFn_handler, deleteDbMessageLog_createServerFn_handler, disconnectBaileys_createServerFn_handler, getBaileysStatus_createServerFn_handler, getDbAttendance_createServerFn_handler, getDbFees_createServerFn_handler, getDbMaterials_createServerFn_handler, getDbSettings_createServerFn_handler, getDbStudents_createServerFn_handler, getMessageLogStats_createServerFn_handler, getMessageLogs_createServerFn_handler, getRegistrationPdf_createServerFn_handler, isDbConfigured_createServerFn_handler, loginAdmin_createServerFn_handler, sendBulkAttendanceAlerts_createServerFn_handler, sendFeeOverdueReminders_createServerFn_handler, sendMonthlyFeeReminders_createServerFn_handler, sendPasswordResetOtp_createServerFn_handler, sendWhatsAppAlert_createServerFn_handler, sendWhatsAppReceipt_createServerFn_handler, syncDbAttendance_createServerFn_handler, syncDbFees_createServerFn_handler, syncDbMaterials_createServerFn_handler, syncDbSettings_createServerFn_handler, syncDbStudents_createServerFn_handler, verifyOtpAndResetPassword_createServerFn_handler };
