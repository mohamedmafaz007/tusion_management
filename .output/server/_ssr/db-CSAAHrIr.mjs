...
//#region node_modules/.nitro/vite/services/ssr/assets/db-CSAAHrIr.js
...
var sendWhatsAppAlert = createServerFn({ method: "POST" }).validator((data) => data).handler(sendWhatsAppAlert_createServerFn_handler, async ({ data }) => {
	...
	if (!template) template = status === "Welcome" ? `Dear Parent, thank you for registering [student_name] at Vishwa Tuition Center.` : `Dear Parent, your child [student_name] was marked ${status} today.`;
...
async function getWhatsAppService() {
	if (typeof window === "undefined") return await import("./whatsappService-CuopoLYW.mjs");
	return null;
}
